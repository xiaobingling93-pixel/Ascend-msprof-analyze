...
def generate_table_info(base_profiling_info, comp_profiling_info, table):
    ...
    if base_profiling_info.flash_attention_time_fwd or comp_profiling_info.flash_attention_time_fwd:
        headers.append('Flash Attention Time(Forward)(Num)')
        base_col.append(f'{base_profiling_info.flash_attention_time_fwd:.3f}s({base_profiling_info.fa_num_fwd})')
        comp_col.append(f'{comp_profiling_info.flash_attention_time_fwd:.3f}s({comp_profiling_info.fa_num_fwd})')
    if base_profiling_info.flash_attention_time_bwd or comp_profiling_info.flash_attention_time_bwd:
        headers.append('Flash Attention Time(Backward)(Num)')
        base_col.append(f'{base_profiling_info.flash_attention_time_bwd:.3f}s({base_profiling_info.fa_num_bwd})')
        comp_col.append(f'{comp_profiling_info.flash_attention_time_bwd:.3f}s({comp_profiling_info.fa_num_bwd})')
