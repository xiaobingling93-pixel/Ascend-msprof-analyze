...
1. **需求澄清和串讲**
    ...
    确定要做该需求后，首先要明确该需求的**迭代时间**，开发流程需要严格遵守我们的迭代时间，参加该需求的需求澄清以及串讲（我们会安排相应会议）。需求澄清可由DE完成（对齐输入以及流程图），需求串讲需要开发者来完成，串讲时需要准备**设计文档和测试用例**（有文档模板，可以跟SE或者DE联系拿到）。
...
3. **资料编写**
    ...
    目前，如果新增一个分析能力，需要编写文档，然后在[分析特性介绍](../../README.md#分析特性介绍)对应表格中添加一行，链接指向该说明文档，文档位置、命名和格式可参考[module_statistic](module_statistic_instruct.md)。
...
5. **代码检视**
    ...
    代码上库，需要经过检视，可以将链接发到**msprof-analyze代码检视群**，说明该PR的标题，然后@相关人进行检视。修改完检视意见后再次@committer，合代码。
